...
class MySceneGraph {
   ...
   */
     ...
     */
    processNode(nodeID, matrix, material, texture, length_s, length_t) {
        //if is component
        if (this.primitives[nodeID] === undefined) {
            var newMatrix = mat4.create();
            newMatrix = mat4.multiply(newMatrix, matrix, this.components[nodeID]["transformations"]);
            var children = this.components[nodeID]["children"];

            var materials = this.components[nodeID]["materials"];
            var currentMaterial = materials[this.mPresses % materials.length];
            if (currentMaterial == "inherit")
                currentMaterial = material;

            var textureDetails = this.components[nodeID]["texture"];
            ...
            for (var i = 0; i < children.length; i++) {
                ...
                this.processNode(children[i], newMatrix, currentMaterial, currentTexture, current_length_s, current_length_t);
        ...
        //if is primitive
            ...
            if (texture != "none") {
                ...
                this.primitives[nodeID].updateTexCoords(length_s, length_t);
            }
            if (texture == "none") {
    ...
    displayScene() {
        ...
        var rootTexture = this.components[this.idRoot]["texture"];
        var selectedMaterial = this.mPresses % this.components[this.idRoot]["materials"].length;
        var rootMaterial = this.components[this.idRoot]["materials"][selectedMaterial];
        this.processNode(this.idRoot, mat4.create(), rootMaterial, rootTexture);
