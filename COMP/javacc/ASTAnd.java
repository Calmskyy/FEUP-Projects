...
class ASTAnd extends SimpleNode {
  ...
  public void analyzeSemantics(ArrayList<SymbolTable> symbolTables, ArrayList<String> errors){
    ...
    if (!st.hasVariable(first_operand_name)) {
        ...
        if(missingVariable){
          errors.add("ERROR: AND includes a variable that does not exist: " + first_operand_name + ".");
          return;
        }
      }
    }
    ...
    else if (first_operand.toString() == "And" || first_operand.toString() == "Less" || first_operand.toString() == "Not")
      first_operand_type = "Boolean";
    ...
    if (second_operand.toString().equals("Identifier")) {
      ...
      if (!st.hasVariable(second_operand_name)) {
        ...
        if(missingVariable){
          errors.add("ERROR: AND includes a variable that does not exist" +  second_operand_name + ".");
          return;
        }
      }
    }
      ...
      if (second_operand_type.equals("")) {
        errors.add("ERROR: AND includes a function call that does not exist: " + second_operand_name + ".");
        return;
      }       
    }
    ...
    if(first_operand_type.equals("Array") || second_operand_type.equals("Array")){
      errors.add("ERROR: AND cannot use arrays on logic operations.");
      return;
    }
    if (first_operand_type.equals("") || second_operand_type.equals("")) {
      errors.add("ERROR: Incompatible AND using a " + first_operand.toString() + " and a " + second_operand.toString() + ".");
      return;
    }

    //test if the two operands are boolean 
    if(verificationWithExpressions1st == false && verificationWithExpressions2st == false){
      if (!first_operand_type.equals(second_operand_type)){
        errors.add("ERROR: Incompatible AND using " + first_operand_type + " and " + second_operand_type + ".");
        return;
      }else if(!first_operand_type.equals("Boolean") || !second_operand_type.equals("Boolean")){
        errors.add("ERROR: Incompatible AND using " + first_operand_type + " and " + second_operand_type + ".");
        return;
      }
    }else if(second_operand.toString().equals("Dot") && second_operand.jjtGetChild(1).toString().equals("Length")){
      errors.add("ERROR: Incompatible AND using " + first_operand_type + " and " + "Length" + ".");
        return;
    }
    ...
    if (first_init == false || second_init == false) {
      ...
      error.append("ERROR: AND contains uninitialized variable(s): ");
      if (first_init == false)
...
/* JavaCC - OriginalChecksum=c5d40f41a0d18eda59eef7c00a7bc4ea (do not edit this line) */
