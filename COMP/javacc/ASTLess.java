...
class ASTLess extends SimpleNode {
  ...
  public void analyzeSemantics(ArrayList<SymbolTable> symbolTables, ArrayList<String> errors){
    ...
    if (!st.hasVariable(first_operand_name)) {
        ...
        if(missingVariable){
          errors.add("ERROR: LESS includes a variable that does not exist: " + first_operand_name + ".");
          return;
        }
      }
    }
      ...
      if (first_operand_type.equals("")) {
        errors.add("ERROR: LESS includes a function call that does not exist: " + first_operand_name + ".");
        return;
      }       
    }
    ...
    if (second_operand.toString().equals("Identifier")) {
      ...
      if (!st.hasVariable(second_operand_name)) {
        ...
        if(missingVariable){
          errors.add("ERROR: LESS includes a variable that does not exist" +  second_operand_name + ".");
          return;
        }
      }
    }
      ...
      if (second_operand_type.equals("")) {
        errors.add("ERROR: LESS includes a function call that does not exist: " + second_operand_name + ".");
        return;
      }       
    }
    ...
    if(first_operand_type.equals("Array") || second_operand_type.equals("Array")){
      errors.add("ERROR: LESS cannot be used with arrays.");
      return;
    }

    //Can't do operations with booleans
    if(first_operand_type.equals("Boolean") || second_operand_type.equals("Boolean")){
      errors.add("ERROR: LESS cannot be used with boolean values.");
      return;
    }

    if (first_operand_type.equals("") || second_operand_type.equals("")) {
      errors.add("ERROR: Incompatible LESS using a " + first_operand.toString() + " and a " + second_operand.toString() + ".");
      return;
    }
    ...
    if(verificationWithExpressions1st == false && verificationWithExpressions2st == false){
      if (!first_operand_type.equals(second_operand_type)){
        errors.add("ERROR: Incompatible LESS using " + first_operand_type + " and " + second_operand_type + ".");
        return;
      }
    }
...
/* JavaCC - OriginalChecksum=643e1cea44f2073297376512d2dc43ff (do not edit this line) */
