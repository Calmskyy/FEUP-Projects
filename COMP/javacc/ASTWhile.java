...
class ASTWhile extends SimpleNode {
  ...
  public void analyzeSemantics(ArrayList<SymbolTable> symbolTables, ArrayList<String> errors){
    ...
    if (operand.toString() == "Identifier") {
      ...
      if (!st.hasVariable(operand_name)) {
          ...
          if (missingVariable) {
            errors.add("ERROR: While includes a variable that does not exist: " + operand_name + ".");
            return;
          }
      }
      else {
      ...
      if (operand_init == false)
        errors.add("ERROR: While condition contains an uninitialized variable: " + operand_name + ".");
      if (operand_type != "Boolean")
        errors.add("ERROR: While condition is not boolean because of a non-boolean variable: " + operand_name + ".");
      return;
    }
    ...
    if (operand.toString() == "Value") {
      if (!(operand.getVal().equals("true") || operand.getVal().equals("false") || operand.getVal().equals("0") || operand.getVal().equals("1")))
        errors.add("ERROR: While condition is not boolean because of a non-boolean value: " + operand.getVal() + ".");
      return;
    }
    ...
    if (operand.toString().equals("Sum") || operand.toString().equals("Sub") || operand.toString().equals("Mult") || operand.toString().equals("Div")) {
      ...
      if (!lessFound)
        errors.add("ERROR: While condition does not contain a boolean expression.");
      return;
    }
  ...
  public String getCode(ArrayList<SymbolTable> symbolTables, SimpleNode stackMethod){
    ...
    int whileIndex = this.jjtGetParent().getIndex();

    code.append("while_" + whileIndex + ":\n");
    ...
    switch (condition.toString()){
      case "Less":
        ...
        code.append("if_icmpge endwhile_" + whileIndex + "\n");
      break;
      default:
      code.append("");
    }
    ...
    code.append("goto while_" + whileIndex + "\n");
    code.append("endwhile_" + whileIndex + ":\n");
...
/* JavaCC - OriginalChecksum=8b5a8bfb9f280e61ff14c29764213c1c (do not edit this line) */
